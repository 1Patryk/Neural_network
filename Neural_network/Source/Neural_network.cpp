...
{
	Neural_network::Bias = 1;
	Neural_network::Beta = 1;
	Neural_network::Amount_of_data = 1000;
	Neural_network::Number_of_epochs = 100;
	Neural_network::Number_of_input = 2;
	Neural_network::Number_of_output = 1;
	Neural_network::Number_of_hidden_layers = 1;
	Neural_network::Number_of_weights = 2;
	Neural_network::Total_number_of_neurons = 1;
	Neural_network::Learning_rate_factor = 1;
	Neural_network::Input_x1 = new std::vector<double>(Amount_of_data);			// This is probably problematic with more data (memory)
	Neural_network::Input_x2 = new std::vector<double>(Amount_of_data);
	Neural_network::Output_y1 = new std::vector<double>(Amount_of_data);
	Neural_network::Vector_of_weights = new std::vector<double>(Number_of_weights);
	Neural_network::Vector_of_neuron_values = new std::vector<double>(Total_number_of_neurons);
}

void Neural_network::Display_results_counting_time(auto Start, auto Stop)
{
...
void Neural_network::Read_data_MLP_x_x(std::vector<double>& Input_x1,
	...
	std::vector<double>& Output_y1)
{
	...
	std::ifstream file("Add_1000.txt");
...
void Neural_network::Min_max_unipolar_scaling(std::vector<double>& Vector)
{
	auto Start = std::chrono::high_resolution_clock::now();
...
void Neural_network::Min_max_bipolar_scaling(std::vector<double>& Vector)
{
	auto Start = std::chrono::high_resolution_clock::now();
...
void Neural_network::Reversal_min_max_unipolar_scaling()
{
	
}

void Neural_network::Pseudo_random_numbers(std::vector<double>& Vector)
{
	auto Start = std::chrono::high_resolution_clock::now();
...
void Neural_network::Calculating_the_network_MLP_X_X(std::vector<double>& Input_x1,
	std::vector<double>& Input_x2,
	std::vector<double>& Output_y1,
...
int Neural_network::Drawning_plot(std::vector<double>* Graph_value_vector)
{
	bool success;
	...
	if (success) {
		...
		WriteToFile(pngdata, "Plot_MLP_X_X.png");
...
double Neural_network::Unipolar_sigmoidal_function(double e)
{
	return (1.0 / (1.0 + exp(-Beta * e)));
}

void Neural_network::Display_results_for_MLP_x_x()
{
